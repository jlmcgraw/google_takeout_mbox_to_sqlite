...
class MboxMessage:
    ...
    def as_tuple(self) -> tuple:
        return (
            self.message_id,
            self.as_json,
            self.received_at.strftime(SQLITE_DATE_FORMAT),
        )
    ...
    def _get_or_generate_message_id(self, msg_dict: dict) -> str:
        ...
        if not msg_id:
            ...
            logging.warning("Missing Message-ID; using SHA256 digest")
            return digest
    ...
    def _msg_to_dict(self, msg: Message) -> dict:
        ...
        payload: Any
        if msg.is_multipart():
            parts = cast(List[Message], msg.get_payload())
            payload = [self._msg_to_dict(part) for part in parts]
        else:
            content_type = msg.get_content_type()
            data = cast(bytes, msg.get_payload(decode=True) or b"")
            ...
            if content_type.startswith("text/"):
                try:
                    ...
                    logging.warning(
                        "Unknown charset '%s'; using utf-8 fallback", charset
                    )
        ...
        for part in payloads:
            ...
            for msg in root.findall(".//cli:message", ns):
                ...
                if ts_epoch:
                    ...
                    jabber_dt = datetime.strptime(
                        ts_delay.attrib["stamp"], "%Y%m%dT%H:%M:%S"
                    )
                    return jabber_dt.replace(tzinfo=timezone.utc)
                elif ts_google is not None:
                    return datetime.fromtimestamp(
                        int(ts_google.attrib["ms"]) / 1000, tz=timezone.utc
                    )

        logging.warning(
            "No valid timestamp found in Google Chat payload; using current time"
        )
        return datetime.now(timezone.utc)
